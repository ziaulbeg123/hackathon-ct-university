            ...
            <tbody>
              ...
              if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                  $exid = $row['exid'];
                  $exQuery = "SELECT * FROM exm_list WHERE exid='$exid'";
                  $nameResult = mysqli_query($conn, $exQuery);
                  $exam = mysqli_fetch_assoc($nameResult);
              ?>
                <tr>
                  <td><?php echo isset($exam['exname']) ? $exam['exname'] : "Exam not found"; ?></td>
